...
const DepartmentPublications = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5000/api/publications/${departmentName}/publications?role=${adminDetails.role}`)
      .then((response) => {
  ...
  }, [departmentName,adminDetails.role]);

  const handlePublicationClick = (publication) => {
    console.log(publication.publication_id);
    navigate(`/Admin/publication/${publication.publication_id}`); // Navigate to the publication detail page
  };
